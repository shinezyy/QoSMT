...
class Spec06:
    ...
    def gen_proc(self, benchmark_name):
        ...
        exe_sub_dir = '{}/exe/{}{}'.format(full_name, benchmark_name, self.suffix)
        executable = pjoin(self.spec_dir, exe_sub_dir)
        proc.executable = executable

        ref_input_dir = pjoin(self.spec_dir, '{}/data/ref/input/'.format(full_name))

        all_input_dir = pjoin(self.spec_dir, '{}/data/all/input/'.format(full_name))
